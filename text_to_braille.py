...
def is_all_caps(s):
    ...
    #if s is in alphabet and has more than one letter then campare if 
    #capitalizing them would change anything
    if str.isalpha(s) and len(s) > 1:
        if str.upper(s) == s:
            return True   
...
def word_to_braille(word):
    ...
    x = 0
    c = ""
    # write codes that take four different situation into consideration
    if is_all_caps(word):
        c = c + CAPITAL +  "\n\n"+ CAPITAL + "\n\n" #add two specail sign first
        while x < len(word):
            c = c + char_to_braille(word[x])+"\n\n" 
            x += 1
            
    #similar method as the above one is just this time the special sign is
    #added to every single character
    else:
        while x <= (len(word)-1):
            if str.isdigit(word[x]):
                c = c + NUMBER + "\n\n" + char_to_braille(word[x])+"\n\n"
            elif str.isupper(word[x]):
                c = c + CAPITAL + "\n\n" + char_to_braille(word[x])+"\n\n"
                
    #doesn't need special operation for regular lower case letter        
            else:
                c = c + char_to_braille(word[x])+"\n\n"
...
def new_filename(fname, addition):
    ...
    '''
    #oh my dear code, please work!
    return fname[:-4] + "_" + addition + fname[-4:] #seperate name and extension
