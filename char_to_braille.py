...
def convert_irregular(c):
    ...
    '..\\noo\\noo'
    '''
    # use if statement and convert them step by step 
    if c == '–' or c == '-':
        return HYPHEN
...
def convert_digit(c):
    ...
    ..
    '''
    #find c's location in DIGIT
    digit = DIGITS.find(c)

    #chage it to ostring using provided function and add the ending
    convert = decade_pattern(digit) + '\n..'
...
def convert_punctuation(c):
    ...
    oo
    '''
    #same stradegy as convert_digit
    pnct = PUNCTUATION.find(c)
...
def decade_ending(dec_num):
    ...
    #similar stradegy as convert_irregular
    if dec_num == 0:
        return '..'
    if dec_num == 1:
        return 'o.'
...
def convert_letter(c):
    ...
    .o
    '''
    # separate the main body and ending

    up = decade_pattern(letter_column(c))

    down = decade_ending(letter_row(c))

    # put them together (it looks nicer than wrtting whole function in one line)
...
def char_to_braille(c):
    ...
    #check if c is any of the types that has a function for and convert c to
    #ostring ( or return c)
    if is_digit(c):
